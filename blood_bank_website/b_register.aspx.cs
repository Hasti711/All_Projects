...
namespace blood_bank
{
    public partial class b_register : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if(Session["request"].ToString() == b_r_em.Value)
            {
        ...
        protected void b_r_fun(object sender,EventArgs e)
        {
            ...
            string ans = iblood_BankClient.b_bk_reg(b_r_nm.Value,b_r_gdoc.SelectedIndex.ToString(), d1, b_r_add.Value,b_r_st.Value, b_r_city.Value,"none", temp1,b_r_em.Value, temp2);
            if (ans == "index.aspx")
            {
                Session["u_email"] = b_r_em.Value;
                Session["update"] = "blood bank";
                Session["request"] = b_r_em.Value;
